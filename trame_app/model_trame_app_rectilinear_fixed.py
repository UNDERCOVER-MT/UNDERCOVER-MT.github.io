...
if not os.path.exists(model_path):
    raise FileNotFoundError(
        ...
        "Please put your file there and name it exactly: model.vtk"
    )
...
# Initialize color limits AFTER state is defined
state.clim = [float(default_min), float(default_max)]
...
def reset_to_model_view():
    renderer.ResetCamera()
    cam = renderer.GetActiveCamera()
    cam.SetFocalPoint(cx, cy, cz)
    cam.SetPosition(cx, cy - 2.5 * R, cz + 1.2 * R)
    cam.SetViewUp(0, 0, 1)
    renderer.ResetCameraClippingRange()
    ctrl.view_update()


ctrl.reset_to_model_view = reset_to_model_view
...
@state.change("clim")
def _update_clim(clim, **kwargs):
    vmin, vmax = float(clim[0]), float(clim[1])

    # Safety: avoid inverted/zero ranges
    if vmax <= vmin:
        return

    # This LUT is shared by mesh + slices + contour in your pipeline
    lut = mesh_actor.GetMapper().GetLookupTable()
    lut.SetRange(vmin, vmax)
    lut.Build()

    # Keep scalar bar synced
...
def _mesh_color(mesh_color_array_idx, **kwargs):
    ...
    # Update the default allowed range to that array’s range
    new_min, new_max = array.get("range")
    state.clim = [float(new_min), float(new_max)]

    # Apply coloring to all actors
...
def standard_buttons():
    ...
    with vuetify.VBtn(icon=True, click=ctrl.reset_to_model_view):
...
def mesh_card():
    with ui_card(title="Mesh", ui_name="mesh"):
        ...
        # Color limits slider (min/max) -> controls LUT range / scalar bar
        vuetify.VRangeSlider(
            v_model=("clim", [float(default_min), float(default_max)]),
            min=float(default_min),
            max=float(default_max),
            step=(
                (float(default_max) - float(default_min)) / 200.0
                if float(default_max) > float(default_min)
                else 1.0
            ),
            label="Color limits (min / max)",
            thumb_label=True,
            dense=True,
            hide_details=True,
            classes="mt-2",
        ...
        with vuetify.VRow(dense=True, classes="pt-1"):
            with vuetify.VCol(cols="6"):
                vuetify.VBtn(
                    ...
                    small=True,
                    block=True,
                    click=f"clim=[{float(default_min)}, {float(default_max)}]",
                )
            with vuetify.VCol(cols="6"):
                vuetify.VBtn(
                    "Swap",
